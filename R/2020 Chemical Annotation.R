...
list.files(pattern="h.*s1_HFT.csv$") 
list.filenames <- list.files(pattern="h.*s1_HFT.csv$")
list.filenames
...
setwd("/Users/ethan_j_li/OneDrive - Emory University/Research/2020 HFT/HILIC 2020 HFT Annotations")
...
list.files(pattern="h.*s2_HFT.csv$") 
list.filenames <- list.files(pattern="h.*s2_HFT.csv$")
list.filenames
...
list.files(pattern="c.*s1_HFT.csv$") 
list.filenames <- list.files(pattern="c.*s1_HFT.csv$")
list.filenames
...
setwd("/Users/ethan_j_li/OneDrive - Emory University/Research/2020 HFT/C18 2020 HFT Annotations")
...
list.files(pattern="c.*s2_HFT.csv$") 
list.filenames <- list.files(pattern="c.*s2_HFT.csv$")
list.filenames
...
for (i in 1:5){
  ...
  combined_list[[a]]$origin <- str_sub(list.filenames2[a],1,-25)
  combined_list[[b]]$origin <- str_sub(list.filenames2[b],1,-25)
  test <- rbind(combined_list[[a]],combined_list[[b]])
  sigtest <-test[test[,10]<0.01,]
  #test <- test[order(test[,3],test[,10],decreasing=FALSE),]
  sigtest<-sigtest[!duplicated(sigtest[,2]),] 
  
  write.csv(sigtest,paste0(str_sub(list.filenames2[a],1,-25),"_combined_annotation.csv"))
}
...
write.csv(hilic_combined_annotation,"HILIC HFT combo annotation.csv")
...
for (i in 1:5){
  ...
  print(list.filenames2[b])
  test <- rbind(combined_list[[a]],combined_list[[b]])
  test$origin <- str_sub(list.filenames2[a],1,-25)
  sigtest <-test[test[,10]<0.01,]
  sigtest<-sigtest[!duplicated(sigtest[,2]),] 
  write.csv(sigtest,paste0(str_sub(list.filenames2[a],1,-19),"_combined_annotation.csv"))
}
...
write.csv(C18_combined_annotation,"C18 HFT combo annotation.csv")
