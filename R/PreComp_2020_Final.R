...
#Get features detected in C18 Mode
...
demographicsbystudyid<-read.csv(here::here("Data","MPTB_AA Cohort Data_N547_addZscore.csv"))
...
#Extract subjectid from Sample.ID in hilicID, values of which = substrings of variable Sample.ID 
...
hilicID$subjectid<-substr(hilicID$Sample.ID,1,5)
...
hilicID$stage<-substr(hilicID$Sample.ID,7,7)
...
###Merge into a complete SID PID Demographic Table
...
demographics.IDs.total <- merge(hilicID,demographicsbystudyid,by=c("subjectid"))
...
HRt$File.Name 
table(HRt$File.Name %in% rownames(HRt)) #TRUE 
                                        # 758 
...
table(total$File.Name %in% HRt$File.Name) #FALSE  TRUE 
                                           #1300   650 
Metotal<-merge(total,HRt, by="File.Name")
#Metotal: 190 variables in total + 13617 variables in HRt - 1 overlap (File.name) = 13806 variables in Metotal
#Metabolic features start at Metotal col# 191 = metabolic feature 1
...
table(Metotal$stage)    # -   1   2   P 
                        # 1 330 270  49 

###Metotals is the Final Dataset, contains basic column info, demographics, HILIC column results
...
#colnames(Metotal[,13897:13902])  ##metabolic feature data ends at 13806; 13616 metabolic features (V1 - V13616)
colnames(Metotal[,13801:13808])

#Create metotal containing columns 1-190 of Metotal, gast, and nullp
#AKA only demographic info, gestational age, nulliparity
metotaldemo<-Metotal[,1:190]
metotaldemo$gast<-Metotal$gast
metotaldemo$nullp<-Metotal$nullp
...
####################################
table(metotaldemo$stage) # stage1: 330 / stage2: 270 / -:1 / P: 49 
...
#AKA all observations who either attended 1 or 2 visits
metotalmis<-subset(metotaldemo,stage==1| stage==2) #600 observations, exclude 1 - and 49 P
...
#birth outcomes of Early Term, Full Term, or Preterm birth, excluding 1 Stillbirth
#metotalnmis: demographics and birth outcomes for all stage 1 or 2 visits ending in ET, FT, or Preterm birth
metotalnmis<-subset(metotalmis,birthoutcome=="EarlyTerm" | birthoutcome=="FullTerm" |
                      birthoutcome=="Preterm")    # 599 obs in total, excluding 1 Preterm-STILLBIRTH, presumably aborted


table(metotalnmis$stage) # 329 stage1 and 270 stage 2  #metotalnmis for later modeling
...
dupes <- data.frame(table(metotalnmis$subjectid)) #E0586 Present 3 times: impossible
...
table(table(metotalnmis$subjectid))
                                    #267 appear twice, 64 appear once
...
a$sub_use <- 0
a$sub_use <- ifelse(a$TobaccoUse_MR ==1 | a$TobaccoUse_MRorSR ==1 | a$tobacco.AnyInPreg ==1 |
                                a$AlcoholUse_MR ==1 | a$AlcoholUse_MRorSR | a$alcohol.AnyInPreg |
                                a$MarijuanaUse_MR ==1 | a$MarijuanaUse_MRorSR | a$marijuana.AnyInPreg, 1, 0)
####Table 1####
#Get a list of variables in a
dput(names(a))
amod <- a
table(amod$g_htn)
amod$g_htn <- ifelse(amod$g_htn == 2,1,
                             ifelse(amod$g_htn==1,1,0))
...
catVars<-c("Education_4.level","income","PregInsurance_2.level",
           ...
           "Sex","g_htn","g_dm","PROM","Comp_gHTNvsHealthyFullTerm","Comp_gDMvsHealthyFullTerm","Comp_PROMvsHealthyFullTerm","Comp_IUGRvsHealthyFullTerm","mode","birthoutcome","TobaccoUse_MR","AlcoholUse_MR","sub_use")

#Create Table One for continuous variables from a listed in contVars
library(tableone)
tab0<-CreateTableOne(vars=contVars, data = amod)
...
catVars<-c("Comp_SpPTBvsALLFullTerm","Comp_AllPTBvsHealthyFullTerm","Comp_InducedPTBvsHealthyFullTerm", "Comp_SpPTBvsHealthyFullTerm",
           "Comp_AllETBvsHealthyFullTerm","Comp_SpETBvsHealthyFullTerm", "Comp_InducedETBvsHealthyFullTerm","Comp_SpPTBSpETBvsHealthyFullTerm","g_htn","Comp_IUGRvsHealthyFullTerm","g_dm")
...
Metotalnmis<-Metotalnmis[!Metotalnmis$Sample.ID=="E0586-1-Red2.1_007",]

#metotalnmis[,288:13903]<-log2(Metotalnmis[,285:13808])
#log transform intensities of metabolic features
metotalnmis[,194:13809]<-log2(Metotalnmis[,191:13806])
...
names(metotalnmis[,1:193])
metotalnmis$sub_use <- 0
metotalnmis$sub_use <- ifelse(metotalnmis$TobaccoUse_MR ==1 | metotalnmis$TobaccoUse_MRorSR ==1 | metotalnmis$tobacco.AnyInPreg ==1 |
                                metotalnmis$AlcoholUse_MR ==1 | metotalnmis$AlcoholUse_MRorSR | metotalnmis$alcohol.AnyInPreg |
                                metotalnmis$MarijuanaUse_MR ==1 | metotalnmis$MarijuanaUse_MRorSR | metotalnmis$marijuana.AnyInPreg, 1, 0)
table(metotalnmis$sub_use, metotalnmis$TobaccoUse_MR)


####Stage 1 HILIC####
metotals1<-subset(metotalnmis,metotalnmis$stage==1)
#Why do we only analyze stage 1 visits?
setwd("C:/Users/EJLI4/OneDrive - Emory University/Research/2020 HFT+Sub+Gast/HILIC 2020 Final Regressions")
list.files()
...
for (i in 1:nrow(hilic_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals1[,i+193] ~as.factor(pre_complication)+as.factor(Sex)+age_enrollment+as.factor(nullp)
                 +as.factor(Education_4.level)+FirstPrenatalBMI+gast+as.factor(sub_use), data=metotals1)
      h_pre_complication_s1[i,]<-summary(lmfit)$coefficients[2,]
      if(i%%100 ==0) {print(i)}
    }, error=function(e){})
}
#h_pre_complication_s1_corr <- data.frame(matrix(nrow = nrow(h_pre_complication_s1),ncol=4))
#h_pre_complication_s1_corr[,1]<-h_pre_complication_s1$Est
#h_pre_complication_s1_corr[,2]<-h_pre_complication_s1$Std
#h_pre_complication_s1_corr[,3]<-h_pre_complication_s1$P
#h_pre_complication_s1_corr[,4]<-h_pre_complication_s1$`t-value`
#colnames(h_pre_complication_s1_corr)<-c("Est","Std","P","t-value")
h_pre_complication_s1$pfdr<-p.adjust(h_pre_complication_s1$P,method="BH")
write.csv(h_pre_complication_s1,"h_pre_complication_s1_Final.csv")
...
for (i in 1:nrow(hilic_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals1[,i+193] ~as.factor(Comp_gHTNvsHealthyFullTerm) +as.factor(Sex)+age_enrollment+as.factor(nullp)
                 +as.factor(Education_4.level)+FirstPrenatalBMI+gast+as.factor(sub_use), data=metotals1)
...
write.csv(h_g_htn_s1,"h_g_htn_s1_Final.csv")

#Rerun w/modified ghtn
metotals1mod <- metotals1
table(metotals1mod$g_htn)
metotals1mod$g_htn <- ifelse(metotals1mod$g_htn == 2,1,
                             ifelse(metotals1mod$g_htn==1,1,0))

h_g_htn_s1mod<-data.frame(matrix(nrow = nrow(hilic_results), ncol = 4))
colnames(h_g_htn_s1mod)<-c("Est","Std","t-value","P")

for (i in 1:nrow(hilic_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals1mod[,i+193] ~g_htn+as.factor(Sex)+age_enrollment+as.factor(nullp)+as.factor(Education_4.level)+FirstPrenatalBMI, data=metotals1mod)
      h_g_htn_s1mod[i,]<-summary(lmfit)$coefficients[2,]
      if(i%%100 ==0) {print(i)}
    }, error=function(e){})
}
h_g_htn_s1mod$pfdr<-p.adjust(h_g_htn_s1mod$P,method="BH")
#h_InducedPTBETBvsHealthyFullTerm_s1<-cbind(h20[,c(1:2)],h_InducedPTBETBvsHealthyFullTerm_s1)
write.csv(h_g_htn_s1mod,"h_g_htn_s1mod.csv")
...
for (i in 1:nrow(hilic_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals1[,i+193] ~as.factor(Comp_gDMvsHealthyFullTerm)+as.factor(Sex)+age_enrollment+as.factor(nullp)
                 +as.factor(Education_4.level)+FirstPrenatalBMI+gast+as.factor(sub_use), data=metotals1)
...
#h_InducedPTBETBvsHealthyFullTerm_s1<-cbind(h20[,c(1:2)],h_InducedPTBETBvsHealthyFullTerm_s1)
write.csv(h_g_dm_s1,"h_g_dm_s1_Final.csv")
...
for (i in 1:nrow(hilic_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals1[,i+193] ~as.factor(Comp_PROMvsHealthyFullTerm)+as.factor(Sex)+age_enrollment+as.factor(nullp)
                 +as.factor(Education_4.level)+FirstPrenatalBMI+gast+as.factor(sub_use), data=metotals1)
...
#h_InducedPTBETBvsHealthyFullTerm_s1<-cbind(h20[,c(1:2)],h_InducedPTBETBvsHealthyFullTerm_s1)
write.csv(h_PROM_s1,"h_PROM_s1_Final.csv")
...
for (i in 1:nrow(hilic_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals1[,i+193] ~as.factor(Comp_IUGRvsHealthyFullTerm)+as.factor(Sex)+age_enrollment+as.factor(nullp)
                 +as.factor(Education_4.level)+FirstPrenatalBMI+gast+as.factor(sub_use), data=metotals1)
...
#h_InducedPTBETBvsHealthyFullTerm_s1<-cbind(h20[,c(1:2)],h_InducedPTBETBvsHealthyFullTerm_s1)
write.csv(h_IUGR_s1,"h_IUGR_s1_Final.csv")
...
for (i in 1:nrow(hilic_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals2[,i+193] ~as.factor(pre_complication)+as.factor(Sex)+age_enrollment+as.factor(nullp)
                 +as.factor(Education_4.level)+FirstPrenatalBMI+gast+as.factor(sub_use), data=metotals2)
...
write.csv(h_pre_complication_s2,"h_pre_complication_s2_Final.csv")
...
for (i in 1:nrow(hilic_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals2[,i+193] ~as.factor(Comp_gHTNvsHealthyFullTerm)+as.factor(Sex)+age_enrollment+as.factor(nullp)
                 +as.factor(Education_4.level)+FirstPrenatalBMI+gast+as.factor(sub_use), data=metotals2)
...
#h_InducedPTBETBvsHealthyFullTerm_s1<-cbind(h20[,c(1:2)],h_InducedPTBETBvsHealthyFullTerm_s1)
write.csv(h_g_htn_s2,"h_g_htn_s2_Final.csv")

#modified h_g_htn_S2
metotals2mod <- metotals2
table(metotals2mod$g_htn)
metotals2mod$g_htn <- ifelse(metotals2mod$g_htn == 2,1,
                             ifelse(metotals2mod$g_htn==1,1,0))

h_g_htn_s2mod<-data.frame(matrix(nrow = nrow(hilic_results), ncol = 4))
colnames(h_g_htn_s2mod)<-c("Est","Std","t-value","P")

for (i in 1:nrow(hilic_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals2mod[,i+193] ~g_htn+as.factor(Sex)+age_enrollment+as.factor(nullp)+as.factor(Education_4.level)+FirstPrenatalBMI, data=metotals2mod)
      h_g_htn_s2mod[i,]<-summary(lmfit)$coefficients[2,]
      if(i%%100 ==0) {print(i)}
    }, error=function(e){})
}
h_g_htn_s2mod$pfdr<-p.adjust(h_g_htn_s2mod$P,method="BH")
#h_InducedPTBETBvsHealthyFullTerm_s1<-cbind(h20[,c(1:2)],h_InducedPTBETBvsHealthyFullTerm_s1)
write.csv(h_g_htn_s2mod,"h_g_htn_s2mod.csv")
...
for (i in 1:nrow(hilic_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals2[,i+193] ~as.factor(Comp_gDMvsHealthyFullTerm)+as.factor(Sex)+age_enrollment+as.factor(nullp)
                 +as.factor(Education_4.level)+FirstPrenatalBMI+gast+as.factor(sub_use), data=metotals2)
...
#h_InducedPTBETBvsHealthyFullTerm_s1<-cbind(h20[,c(1:2)],h_InducedPTBETBvsHealthyFullTerm_s1)
write.csv(h_g_dm_s2,"h_g_dm_s2_Final.csv")
...
for (i in 1:nrow(hilic_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals2[,i+193] ~as.factor(Comp_PROMvsHealthyFullTerm)+as.factor(Sex)+age_enrollment+as.factor(nullp)
                 +as.factor(Education_4.level)+FirstPrenatalBMI+gast+as.factor(sub_use), data=metotals2)
...
#h_InducedPTBETBvsHealthyFullTerm_s1<-cbind(h20[,c(1:2)],h_InducedPTBETBvsHealthyFullTerm_s1)
write.csv(h_PROM_s2,"h_PROM_s2_Final.csv")
...
for (i in 1:nrow(hilic_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals2[,i+193] ~as.factor(Comp_IUGRvsHealthyFullTerm)+as.factor(Sex)+age_enrollment+as.factor(nullp)
                 +as.factor(Education_4.level)+FirstPrenatalBMI+gast+as.factor(sub_use), data=metotals2)
...
#h_InducedPTBETBvsHealthyFullTerm_s1<-cbind(h20[,c(1:2)],h_InducedPTBETBvsHealthyFullTerm_s1)
write.csv(h_IUGR_s2,"h_IUGR_s2_Final.csv")
#######################################
##########C18####
#CPid<-CPid20
c18ID <- c18_ids
...
c18ID$subjectid<-substr(c18ID$Sample.ID,1,5)
#CPid$stage<-substr(CPid$Sample.ID,7,7)
c18ID$stage <- substr(c18ID$Sample.ID,7,7)
...
Metotal2<-merge(total2,CRt, by="File.Name")
...
metotal2demo<-Metotal2[,1:190] #Demographics+ C18 basic column info + gestational age + nulliparity
metotal2demo$gast<-Metotal2$gast
metotal2demo$nullp<-Metotal2$nullp
#table(metotal2$stage) 
#Demographics+ C18 basic column info + gestational age + nulliparity only inc stage 1 + stage 2
metotalmis2<-subset(metotal2demo,stage==1| stage==2)
# metotalnmis<-subset(metotalmis,Comp_SpPTBvsALLFullTerm !=" " 
#                     | Comp_SpPTBSpETBvsHealthyFullTerm !=" " 
#                     | Comp_SpPTBvsHealthyFullTerm !=" "
...
# 1   2 
#64 267  267 appear twice, 64 appear once, matches HILIC data
...
metotalnmis2[,194:12093]<-log2(Metotalnmis2[,191:12090])   
...
####C18 Stage 1####
setwd("C:/Users/EJLI4/OneDrive - Emory University/Research/2020 HFT+Sub+Gast/C18 2020 Final Regressions")
#Question: why separate analyses for stage 1 and stage 2 data?
metotalnmis2$sub_use <- 0
metotalnmis2$sub_use <- ifelse(metotalnmis2$TobaccoUse_MR ==1 | metotalnmis2$TobaccoUse_MRorSR ==1 | metotalnmis2$tobacco.AnyInPreg ==1 |
                                metotalnmis2$AlcoholUse_MR ==1 | metotalnmis2$AlcoholUse_MRorSR | metotalnmis2$alcohol.AnyInPreg |
                                metotalnmis2$MarijuanaUse_MR ==1 | metotalnmis2$MarijuanaUse_MRorSR | metotalnmis2$marijuana.AnyInPreg, 1, 0)
table(metotalnmis2$sub_use, metotalnmis2$TobaccoUse_MR)
metotals1_2<-subset(metotalnmis2,metotalnmis2$stage==1) #267+64-3

#preg complications regression
c_pre_complication_s1<-data.frame(matrix(nrow = nrow(c18_results), ncol = 4))
colnames(c_pre_complication_s1)<-c("Est","Std","t-value","P")
#lmfit<-lm( metotals1_2[,1231] ~pre_complication+as.factor(Sex)+age_enrollment+as.factor(nullp)+as.factor(Education_4.level)+FirstPrenatalBMI, data=metotals1_2)
  ...
  tryCatch(
    {
      ...
      lmfit<-lm( metotals1_2[,i+193] ~as.factor(pre_complication)+as.factor(Sex)+age_enrollment+as.factor(nullp)
                 +as.factor(Education_4.level)+FirstPrenatalBMI+gast+as.factor(sub_use), data=metotals1_2)
...
write.csv(c_pre_complication_s1,"c_pre_complication_s1_Final.csv")
...
for (i in 1:nrow(c18_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals1_2[,i+193] ~as.factor(Comp_gHTNvsHealthyFullTerm)+as.factor(Sex)+age_enrollment+as.factor(nullp)
                 +as.factor(Education_4.level)+FirstPrenatalBMI+gast+as.factor(sub_use), data=metotals1_2)
...
#h_InducedPTBETBvsHealthyFullTerm_s1<-cbind(h20[,c(1:2)],h_InducedPTBETBvsHealthyFullTerm_s1)
write.csv(c_g_htn_s1,"c_g_htn_s1_Final.csv")

metotals1_2mod <- metotals1_2
table(metotals1_2mod$g_htn)
metotals1_2mod$g_htn <- ifelse(metotals1_2mod$g_htn == 2,1,
                             ifelse(metotals1_2mod$g_htn==1,1,0))

c_g_htn_s1mod<-data.frame(matrix(nrow = nrow(c18_results), ncol = 4))
colnames(c_g_htn_s1mod)<-c("Est","Std","t-value","P")

for (i in 1:nrow(c18_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals1_2mod[,i+193] ~g_htn+as.factor(Sex)+age_enrollment+as.factor(nullp)+as.factor(Education_4.level)+FirstPrenatalBMI, data=metotals1_2mod)
      c_g_htn_s1mod[i,]<-summary(lmfit)$coefficients[2,]
      if(i%%100 ==0) {print(i)}
    }, error=function(e){})
}
c_g_htn_s1mod$pfdr<-p.adjust(c_g_htn_s1mod$P,method="BH")
#h_InducedPTBETBvsHealthyFullTerm_s1<-cbind(h20[,c(1:2)],h_InducedPTBETBvsHealthyFullTerm_s1)
write.csv(c_g_htn_s1mod,"c_g_htn_s1mod.csv")
...
for (i in 1:nrow(c18_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals1_2[,i+193] ~as.factor(Comp_gDMvsHealthyFullTerm)+as.factor(Sex)+age_enrollment+as.factor(nullp)
                 +as.factor(Education_4.level)+FirstPrenatalBMI+gast+as.factor(sub_use), data=metotals1_2)
...
#h_InducedPTBETBvsHealthyFullTerm_s1<-cbind(h20[,c(1:2)],h_InducedPTBETBvsHealthyFullTerm_s1)
write.csv(c_g_dm_s1,"c_g_dm_s1_Final.csv")
...
for (i in 1:nrow(c18_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals1_2[,i+193] ~as.factor(Comp_PROMvsHealthyFullTerm)+as.factor(Sex)+age_enrollment+as.factor(nullp)
                 +as.factor(Education_4.level)+FirstPrenatalBMI+gast+as.factor(sub_use), data=metotals1_2)
...
#h_InducedPTBETBvsHealthyFullTerm_s1<-cbind(h20[,c(1:2)],h_InducedPTBETBvsHealthyFullTerm_s1)
write.csv(c_PROM_s1,"c_PROM_s1_Final.csv")
...
for (i in 1:nrow(c18_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals1_2[,i+193] ~as.factor(Comp_IUGRvsHealthyFullTerm)+as.factor(Sex)+age_enrollment+as.factor(nullp)
                 +as.factor(Education_4.level)+FirstPrenatalBMI+gast+as.factor(sub_use), data=metotals1_2)
...
#h_InducedPTBETBvsHealthyFullTerm_s1<-cbind(h20[,c(1:2)],h_InducedPTBETBvsHealthyFullTerm_s1)
write.csv(c_IUGR_s1,"c_IUGR_s1_Final.csv")
...
for (i in 1:nrow(c18_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals2_2[,i+193] ~as.factor(Comp_gHTNvsHealthyFullTerm)+as.factor(Sex)+age_enrollment+as.factor(nullp)
                 +as.factor(Education_4.level)+FirstPrenatalBMI+gast+as.factor(sub_use), data=metotals2_2)
...
#h_InducedPTBETBvsHealthyFullTerm_s1<-cbind(h20[,c(1:2)],h_InducedPTBETBvsHealthyFullTerm_s1)
write.csv(c_g_htn_s2,"c_g_htn_s2_Final.csv")

metotals2_2mod <- metotals2_2
table(metotals2_2mod$g_htn)
metotals2_2mod$g_htn <- ifelse(metotals2_2mod$g_htn == 2,1,
                               ifelse(metotals2_2mod$g_htn==1,1,0))

c_g_htn_s2mod<-data.frame(matrix(nrow = nrow(c18_results), ncol = 4))
colnames(c_g_htn_s2mod)<-c("Est","Std","t-value","P")

for (i in 1:nrow(c18_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals2_2mod[,i+193] ~g_htn+as.factor(Sex)+age_enrollment+as.factor(nullp)+as.factor(Education_4.level)+FirstPrenatalBMI, data=metotals2_2mod)
      c_g_htn_s2mod[i,]<-summary(lmfit)$coefficients[2,]
      if(i%%100 ==0) {print(i)}
    }, error=function(e){})
}
c_g_htn_s2mod$pfdr<-p.adjust(c_g_htn_s2mod$P,method="BH")
#h_InducedPTBETBvsHealthyFullTerm_s1<-cbind(h20[,c(1:2)],h_InducedPTBETBvsHealthyFullTerm_s1)
write.csv(c_g_htn_s2mod,"c_g_htn_s2mod.csv")
...
for (i in 1:nrow(c18_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals2_2[,i+193] ~as.factor(Comp_gDMvsHealthyFullTerm)+as.factor(Sex)+age_enrollment+as.factor(nullp)
                 +as.factor(Education_4.level)+FirstPrenatalBMI+gast+as.factor(sub_use), data=metotals2_2)
...
#h_InducedPTBETBvsHealthyFullTerm_s1<-cbind(h20[,c(1:2)],h_InducedPTBETBvsHealthyFullTerm_s1)
write.csv(c_g_dm_s2,"c_g_dm_s2_Final.csv")
...
for (i in 1:nrow(c18_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals2_2[,i+193] ~as.factor(Comp_PROMvsHealthyFullTerm)+as.factor(Sex)+age_enrollment+as.factor(nullp)
                 +as.factor(Education_4.level)+FirstPrenatalBMI+gast+as.factor(sub_use), data=metotals2_2)
...
#h_InducedPTBETBvsHealthyFullTerm_s1<-cbind(h20[,c(1:2)],h_InducedPTBETBvsHealthyFullTerm_s1)
write.csv(c_PROM_s2,"c_PROM_s2_Final.csv")
...
for (i in 1:nrow(c18_results) ) 
{
  tryCatch(
    {
      lmfit<-lm( metotals2_2[,i+193] ~as.factor(Comp_IUGRvsHealthyFullTerm)+as.factor(Sex)+age_enrollment+as.factor(nullp)
                 +as.factor(Education_4.level)+FirstPrenatalBMI+gast+as.factor(sub_use), data=metotals2_2)
...
#h_InducedPTBETBvsHealthyFullTerm_s1<-cbind(h20[,c(1:2)],h_InducedPTBETBvsHealthyFullTerm_s1)
write.csv(c_IUGR_s2,"c_IUGR_s2_Final.csv")
...
for (i in 1:nrow(c18_results) ) 
{
  tryCatch(
    {
      # lmfit<-lm( metotals1_2[,i+193] ~Comp_SpPTBvsALLFullTerm+as.factor(Sex)+age+as.factor(nullp)+as.factor(Education_4.level)+FirstPrenatalBMI, data=metotals1_2)
      lmfit<-lm( metotals2_2[,i+193] ~as.factor(pre_complication)+as.factor(Sex)+age_enrollment+as.factor(nullp)
                 +as.factor(Education_4.level)+FirstPrenatalBMI+gast+as.factor(sub_use), data=metotals2_2)
...
write.csv(c_pre_complication_s2,"c_pre_complication_s2_Final.csv")
...
setwd("C:/Users/EJLI4/OneDrive - Emory University/Research/2020 HFT+Sub+Gast/HILIC 2020 Final Regressions")
list.files(pattern="h.*Final.csv$") 
list.filenames <- list.files(pattern="h.*Final.csv$")
...
write.csv(MWAStatsHILICPreComp,"HILIC 2020 Final MWAS Stats.csv")

MWAStatsHILICgHTNMod <- as.data.frame(matrix(nrow = 4, ncol = 7))
colnames(MWAStatsHILICgHTNMod)<-c("Outcome","FDR0.05","FDR0.20","Raw0.0005", "Raw0.005","Raw0.01","Raw0.05")
for (i in 1:4)
{
  tryCatch(
    {
      MWAStatsHILICgHTNMod[i,1]<-names(lmfit_crude)[i]
      MWAStatsHILICgHTNMod[i,2]<-sum(lmfit_crude[[i]][6]<0.05,na.rm = TRUE)
      MWAStatsHILICgHTNMod[i,3]<-sum(lmfit_crude[[i]][6]<0.20,na.rm = TRUE)
      MWAStatsHILICgHTNMod[i,4]<-sum(lmfit_crude[[i]][5]<0.0005,na.rm = TRUE)
      MWAStatsHILICgHTNMod[i,5]<-sum(lmfit_crude[[i]][5]<0.005,na.rm = TRUE)
      MWAStatsHILICgHTNMod[i,6]<-sum(lmfit_crude[[i]][5]<0.01,na.rm = TRUE)
      MWAStatsHILICgHTNMod[i,7]<-sum(lmfit_crude[[i]][5]<0.05,na.rm = TRUE)
    },error=function(e){})
}

write.csv(MWAStatsHILICgHTNMod,"MWAStatsHILICgHTNMod.csv")

#HILIC PreComp Between stage (S1-S2) per-complication overlaps
...
{
df1 <- as.data.frame(matrix(nrow=nrow(lmfit_crude[[i]]),ncol = 7))
names(df1) <- c("ID","P<0.05?","P<0.01?","P<0.005?","P<0.0005?","BHP<0.2?","BHP<0.05?")
df1$ID <- (lmfit_crude[[i]][1])
df1$`P<0.05?` <- (lmfit_crude[[i]][5]<0.05)
df1$`P<0.01?` <- (lmfit_crude[[i]][5]<0.01)
df1$`P<0.005?` <- (lmfit_crude[[i]][5]<0.005)
df1$`P<0.0005?` <- (lmfit_crude[[i]][5]<0.0005)
df1$`BHP<0.2?`<- (lmfit_crude[[i]][6]<0.2)
df1$`BHP<0.05?`<- (lmfit_crude[[i]][6]<0.05)

df2 <- as.data.frame(matrix(nrow=nrow(lmfit_crude[[i+1]]),ncol = 7))
names(df2) <- c("ID","P<0.05?","P<0.01?","P<0.005?","P<0.0005?","BHP<0.2?","BHP<0.05?")
df2$ID <- (lmfit_crude[[i+1]][1])
df2$`P<0.05?` <- (lmfit_crude[[i+1]][5]<0.05)
df2$`P<0.01?` <- (lmfit_crude[[i+1]][5]<0.01)
df2$`P<0.005?` <- (lmfit_crude[[i+1]][5]<0.005)
df2$`P<0.0005?` <- (lmfit_crude[[i+1]][5]<0.0005)
df2$`BHP<0.2?`<- (lmfit_crude[[i+1]][6]<0.2)
df2$`BHP<0.05?`<- (lmfit_crude[[i+1]][6]<0.05)

overlap[a,1] <- substring(list.filenames[i],1,nchar(list.filenames[i])-7)
overlap[a,2] <- length(df1$ID[df1$`P<0.05?` == TRUE & df2$`P<0.05?` == TRUE])
overlap[a,3] <- length(df1$ID[df1$`P<0.01?` == TRUE & df2$`P<0.01?` == TRUE])
overlap[a,4] <- length(df1$ID[df1$`P<0.005?` == TRUE & df2$`P<0.005?` == TRUE])
overlap[a,5] <- length(df1$ID[df1$`P<0.0005?` == TRUE & df2$`P<0.0005?` == TRUE])
overlap[a,6] <- length(df1$ID[df1$`BHP<0.2?` == TRUE & df2$`BHP<0.2?` == TRUE])
overlap[a,7] <- length(df1$ID[df1$`BHP<0.05?` == TRUE & df2$`BHP<0.05?` == TRUE])
a <- a+1
}

write.csv(overlap,"HILIC 2020 Final Stage 1x2 Overlap.csv")
...
setwd("/Users/ethan_j_li/Documents/Emory/Research/2020/C18 PreComp Regressions 2020")
setwd("C:/Users/EJLI4/OneDrive - Emory University/Research/2020 HFT+Sub+Gast/C18 2020 Final Regressions")
list.files(pattern="c.*Final.csv$") 
list.filenames <- list.files(pattern="c.*Final.csv$")
...
write.csv(MWAStatsC18PreComp,"C18 2020 Final MWAS Stats.csv")

MWAStatsC18gHTNMod <- as.data.frame(matrix(nrow = 4, ncol = 7))
colnames(MWAStatsC18gHTNMod)<-c("Outcome","FDR0.05","FDR0.20","Raw0.0005", "Raw0.005","Raw0.01","Raw0.05")
for (i in 1:4)
{
  tryCatch(
    {
      MWAStatsC18gHTNMod[i,1]<-names(lmfit_crude)[i]
      MWAStatsC18gHTNMod[i,2]<-sum(lmfit_crude[[i]][6]<0.05,na.rm = TRUE)
      MWAStatsC18gHTNMod[i,3]<-sum(lmfit_crude[[i]][6]<0.20,na.rm = TRUE)
      MWAStatsC18gHTNMod[i,4]<-sum(lmfit_crude[[i]][5]<0.0005,na.rm = TRUE)
      MWAStatsC18gHTNMod[i,5]<-sum(lmfit_crude[[i]][5]<0.005,na.rm = TRUE)
      MWAStatsC18gHTNMod[i,6]<-sum(lmfit_crude[[i]][5]<0.01,na.rm = TRUE)
      MWAStatsC18gHTNMod[i,7]<-sum(lmfit_crude[[i]][5]<0.05,na.rm = TRUE)
    },error=function(e){})
}

write.csv(MWAStatsC18gHTNMod,"MWAStatsC18gHTNMod.csv")


#C18 PreComp Between-stage (S1-S2) per-complication Overlap
overlap <- as.data.frame(matrix(nrow=5,ncol=7))
names(overlap) <- c("Comparison", "Overlap P<0.05", "Overlap P<0.01", "Overlap P<0.005", "Overlap P<0.0005","Overlap BHP<0.2","Overlap BHP<0.05")
a <- 1
for (i in seq(from=1,to=9,by=2))
...
write.csv(overlap,"C18 2020 Final Stage 1x2 Overlap.csv")
  ...
  tryCatch(
    {
      ...
      #replace 1 with i
      write.table(ACE_ref2, paste0("C:/Users/EJLI4/OneDrive - Emory University/Research/2020 HFT+Sub+Gast/HILIC 2020 Final Mummichog txt Input/",polname[i],".txt"),sep="\t",row.names=FALSE)
    },error=function(e){})
}
  ...
  tryCatch(
    {
      ...
      write.table(ACE_ref2, paste0("C:/Users/EJLI4/OneDrive - Emory University/Research/2020 HFT+Sub+Gast/C18 2020 Final Mummichog txt Input/",polname[i],".txt"),sep="\t",row.names=FALSE)
    },error=function(e){})
}
...
LVL1.match<-slice(HMSMS,masteroverlap$index.B)
LVL1total<-cbind(LVL1.match,LVL1.sig)
LVL1total<-subset(LVL1total,LVL1total$totalsig>0) #p<0.05
LVL1total<-LVL1total[!duplicated(LVL1total[,2]),]  
write.csv(LVL1total,"MWASs2p_lvl1.csv")
