#HILIC 2017 Stage 1 Pairwise Overlaps
...
for (firstdata in 1:(length(lmfit_crude)-1))
{
  ...
  for(seconddata in a:length(lmfit_crude))
  {
    ...
    overlap[overlaprow,1] <- paste("s1_",substring(list.filenames[firstdata],1,nchar(list.filenames[firstdata])-7), 
                                   ...
                                   sep = "+")
    overlap[overlaprow,2] <- length(df1$X[df1$P <0.05 & df2$P < 0.05 & is.na(df1$P) == F & is.na(df2$P)==F])
    overlap[overlaprow,3] <- length(df1$X[df1$P < 0.01 & df2$P < 0.01 & is.na(df1$P) == F & is.na(df2$P)==F])
    overlap[overlaprow,4] <- length(df1$X[df1$P<0.005 & df2$P<0.005 & is.na(df1$P) == F & is.na(df2$P)==F])
    overlap[overlaprow,5] <- length(df1$X[df1$P<0.0005 & df2$P<0.0005 & is.na(df1$P) == F & is.na(df2$P)==F])
    overlap[overlaprow,6] <- length(df1$X[df1$pdfr<0.2 & df2$pdfr<0.2 & is.na(df1$pdfr) == F & is.na(df2$pdfr)==F])
    overlap[overlaprow,7] <- length(df1$X[df1$pdfr<0.05 & df2$pdfr<0.05 & is.na(df1$pdfr) == F & is.na(df2$pdfr == F)])
...
write.csv(overlap,"HILIC Stage 1 Pairwise 2017 Overlaps.csv")

#############
#############

#HILIC 2017 Stage 2 Pairwise Overlaps 
...
write.csv(overlap,"HILIC Stage 2 Pairwise 2017 Overlaps.csv")

#############
#############

#C18 2017 Stage 1 Pairwise Overlaps
...
write.csv(overlap,"C18 Stage 1 Pairwise 2017 Overlaps.csv")

#############
#############

#C18 2010 Stage 2 Pairwise Overlaps
...
write.csv(overlap,"C18 Stage 2 Pairwise 2017 Overlaps.csv")
